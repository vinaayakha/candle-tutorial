...
pub fn create_2d_tensor() -> Result<Tensor> {
    ...
    Tensor::new(&[[1u32, 2, 3],[4,5,6]], &Device::Cpu)
}
...
pub fn inspect_tensor(t: &Tensor) -> Result<(usize, usize, String)> {
    ...
    let n_dims = t.dims().len();
    let n_elems = t.elem_count();
    let dtype_name = format!("{:?}", t.dtype());
    Ok((n_dims, n_elems, dtype_name))
}
